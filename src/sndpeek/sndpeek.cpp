...
double compute_pow_spacing( int fft_size, double factor );
...
GLdouble g_log_factor = 0;
GLdouble g_pow_factor = 1;
GLboolean g_use_log = true;

// center and zoom
GLdouble g_centering = 0;
GLdouble g_zooming = 1;
GLdouble g_left_trim = 0;
GLdouble g_right_trim = 0;
...
{
    fprintf( stderr, "----------------------------------------------------\n" );
    fprintf( stderr, "sndpeek + wutrfall (1.3c)\n" );
    ...
    fprintf( stderr, "'e' - toggle between linear->LOG and linear->POW freq scaling\n" );
    fprintf( stderr, "'*' - restart file playback (if applicable)\n" );
    ...
    fprintf( stderr, "':' - left trim increase\n" );
    fprintf( stderr, "';' - left trim decrease\n" );
    ...
    fprintf( stderr, "'c' = LOWER log/pow factor of spectrum display\n" );
    fprintf( stderr, "'v' = RAISE log/pow factor of spectrum display\n" );
    fprintf( stderr, "      (log factor of 0.0 is linear frequency\n" );
    fprintf( stderr, "      (log factor of 1.0 is log frequency\n" );
    fprintf( stderr, "      (pow factor of 1.0 is linear display\n" );
...
void usage()
{
    ...
    fprintf( stderr, "  number options: timescale|freqscale|lissscale|logfactor|powfactor|\n" );
    fprintf( stderr, "                  spacing|zpos|dzpos|depth|preview|yview|\n" );
    fprintf( stderr, "                  rotatem|rotatek|begintime|ds|ltrim\n" );
    fprintf( stderr, "   other options: nodisplay|print\n" );
    fprintf( stderr, "\n" );
    fprintf( stderr, "example:\n" );
    fprintf( stderr, "    sndpeek --fullscreen:ON --features:OFF --spacing:.05\n" );
    fprintf( stderr, "\n" );
    fprintf( stderr, "sndpeek version: 1.3c\n" );
...
int main( int argc, char ** argv )
{
    ...
    for( int i = 1; i < argc; i++ )
    {
        if( !strncmp( argv[i], "-", 1 ) )
        {
            ...
            else if( !strncmp(argv[i], "--powfactor:", 12) )
            {
                g_pow_factor = atof( argv[i]+12 ) > 0 ? atof( argv[i]+12 ) : g_pow_factor;
                g_use_log = false;
            }
            else if( !strncmp(argv[i], "--ltrim:", 8) )
                g_left_trim = ( atof( argv[i]+8 ) > 0 ) && ( atof( argv[i]+8 ) < .5 ) ? atof( argv[i]+8 ) : g_left_trim;
        ...
        double streamTime, RtAudioStreamStatus status, void * data )
{
    ...
    // clear
    memset( outBuffy, 0, numFrames * 2 * sizeof(SAMPLE) );
    ...
    {
        // copy
        memcpy( g_stereo_buffer, inBuffy, numFrames * 2 * sizeof(SAMPLE) );
    ...
    else
    {
        ...
        if( sf_seek(g_sf, 0, SEEK_CUR) < g_sf_info.frames && !g_pause )
        {
            ...
            // play stereo
            memcpy( outBuffy, g_stereo_buffer, numFrames * 2 * sizeof(SAMPLE) );
        ...
        else
        {
            ...
            if( g_wf_delay )
            {
                ...
                memset( outBuffy, 0, 2 * numFrames * sizeof(SAMPLE) );
        }
    }
...
bool initialize_audio( )
{
    ...
    if( !g_filename || g_sndout )
    {
        ...
        {
            // print error message
            cerr << "[sndpeek](via RtAudio): " << e.getMessage() << endl;
            // done
            exit(1);
        }
    }
...
void initialize_graphics()
{
    ...
    if( g_use_log )
        ...
        g_log_space = compute_pow_spacing( g_fft_size / 2, g_pow_factor );
...
void keyboardFunc( unsigned char key, int x, int y )
{
    ...
    case 'v':
        ...
        {
            g_log_factor += .01;
            if( g_log_factor > 1 ) g_log_factor = 1;
            g_log_space = compute_log_spacing( g_fft_size / 2, g_log_factor );
            fprintf( stderr, "[sndpeek]: logfactor:%f\n", g_log_factor );
        }
        else
        {
            g_pow_factor *= .98; //.99985;
            g_log_space = compute_pow_spacing( g_fft_size / 2, g_pow_factor );
            fprintf( stderr, "[sndpeek]: powfactor:%f\n", g_pow_factor );
        }
    break;
    case 'c':
        // compute
        ...
        {
            g_log_factor -= .01;
            if( g_log_factor < 0 ) g_log_factor = 0;
            g_log_space = compute_log_spacing( g_fft_size / 2, g_log_factor );
            fprintf( stderr, "[sndpeek]: logfactor:%f\n", g_log_factor );
        }
        else
    ...
    case 'e':
        g_use_log = !g_use_log;
        fprintf( stderr, "[sndpeek]: frequency axis scaling:[linear<->%s]\n", 
                 g_use_log ? "LOG" : "POW" );
        // compute
        if( g_use_log )
            g_log_space = compute_log_spacing( g_fft_size / 2, g_log_factor );
    ...
    case '<':
        g_centering -= .01;
        if( g_centering < -1 ) g_centering = -1;
        fprintf( stderr, "[sndpeek]: centering:%f\n", g_centering );
        // compute
        if( g_use_log )
            g_log_space = compute_log_spacing( g_fft_size / 2, g_log_factor );
        else
    ...
    case ',':
        g_zooming /= 1.05;
        fprintf( stderr, "[sndpeek]: zooming:%f\n", g_zooming );
        // compute
        if( g_use_log )
            g_log_space = compute_log_spacing( g_fft_size / 2, g_log_factor );
        else
    ...
    case ';':
        g_left_trim -= .001;
        if( g_left_trim < 0 ) g_left_trim = 0;
        fprintf( stderr, "[sndpeek]: left trim:%f\n", g_left_trim );
        // compute
        if( g_use_log )
            g_log_space = compute_log_spacing( g_fft_size / 2, g_log_factor );
        else
    ...
    case ':':
        g_left_trim += .001;
        if( g_left_trim > .5 ) g_left_trim = .5;
        fprintf( stderr, "[sndpeek]: left trim:%f\n", g_left_trim );
        // compute
        if( g_use_log )
            g_log_space = compute_log_spacing( g_fft_size / 2, g_log_factor );
    ...
    case 'p':
        ...
        fprintf( stderr, "[sndpeek]: logfactor:%f\n", g_log_factor );
        fprintf( stderr, "[sndpeek]: powfactor:%f\n", g_pow_factor );
        fprintf( stderr, "[sndpeek]: ltrim:%f\n", g_left_trim );
        fprintf( stderr, "[sndpeek]: lissscale:%f\n", g_lissajous_scale );
        fprintf( stderr, "[sndpeek]: lissdelay = %i\n", g_delay );
        fprintf( stderr, "[sndpeek]: zpos:%f\n", g_z );
...
{
    if( ratio <= g_left_trim ) ratio = 0;
    else ratio = (ratio - g_left_trim) / (1.0 - g_left_trim);

    // compute location
    if( ratio < (1.0/g_fft_size) ) return 0; // g_centering/2 * g_zooming * g_fft_size/g_freq_view;
    double offset = -log(1.0/g_fft_size);
    return ((1-power)*ratio + (power)*(offset+log(ratio))/offset) * g_fft_size/g_freq_view;
}
...
//-----------------------------------------------------------------------------
// Name: map_pow_spacing( )
// Desc: ...
//-----------------------------------------------------------------------------
inline double map_pow_spacing( double ratio, double power )
{
    if( ratio <= g_left_trim ) ratio = 0;
    else ratio = (ratio - g_left_trim) / (1.0 - g_left_trim);
...
//-----------------------------------------------------------------------------
// Name: compute_pow_spacing( )
// Desc: ...
//-----------------------------------------------------------------------------
double compute_pow_spacing( int fft_size, double power )
{
    int maxbin = fft_size; // for future in case we want to draw smaller range
    int minbin = 0; // what about adding this one?
    ...
    {
        // compute location
        g_log_positions[i] = map_pow_spacing( (double)i/fft_size, power );
        // normalize, 1 if maxbin == fft_size
        g_log_positions[i] /= pow((double)maxbin/fft_size, power);
    }
...
void displayFunc( )
{
    ...
    glPushMatrix( );
            ...
            if( i == g_wf_delay || !g_freeze || g_wutrfall )
            {
                ...
                if( g_draw[(g_wf+i)%g_depth] )
                {
                    ...
                    for( GLint j = 0; j < g_fft_size/g_freq_view; j++, pt++ )
                    {
                        ...
                        glVertex3f( g_log_positions[j], (double)j/(g_fft_size/g_freq_view) >= g_left_trim ? pt->y : y, d );
                    }
                    glEnd();
        ...
        if( g_draw_features )
        {
            ...
            float centroid_x = g_use_log ?
                map_log_spacing( centroid_val/SND_MARSYAS_SIZE, g_log_factor ) :
                map_pow_spacing( centroid_val/SND_MARSYAS_SIZE, g_pow_factor );
            ...
            float rolloff_x = g_use_log ?
                map_log_spacing( rolloff_val/SND_MARSYAS_SIZE, g_log_factor ) :
                map_pow_spacing( rolloff_val/SND_MARSYAS_SIZE, g_pow_factor );
            ...
            float rolloff2_x = g_use_log ?
                map_log_spacing( rolloff2_val/SND_MARSYAS_SIZE, g_log_factor ) :
                map_pow_spacing( rolloff2_val/SND_MARSYAS_SIZE, g_pow_factor );                
