...
char s_stream[1024];
int sndfile_count = 0;
SF_INFO  sfinfo, input_sfinfo;
SNDFILE  *output_sndfile;
SNDFILE  *input_sndfile;
string output_sndfile_name = "pvc_output_";
int input_sndfile_count = 0;
string input_sndfile_name;
int g_cross_synthesize = FALSE;
pvc_data * g_pvc2;
...
{
    if (argc != 1) {
        input_sndfile_name = argv[1];
        input_sndfile_count = 1;
    }
...
bool initialize_audio( )
{
    ...
    g_pvc2 = pv_create( g_window_size, 1024, 0 );
    pv_set_window( g_pvc2, PV_HANNING );
...
void keyboardFunc( unsigned char key, int x, int y )
{
    ...
    case 'd':
        ...
        break;
    case 'q':
        if (output_sndfile != NULL) {
            cerr << "done saving sndfile\n";
    ...
    case 'o':
        if(output_sndfile == NULL) {
            cerr << "saving sndfile\n";
            sprintf( s_stream, sndfile_count >= 10000 ? "%i" : "%.4i", sndfile_count++ );
            if (! (output_sndfile = sf_open((output_sndfile_name+s_stream+".wav").c_str(), SFM_WRITE, &sfinfo)))
                cerr << "can't open output file: " << (output_sndfile_name+s_stream+".wav")  << endl;
        } else {
            cerr << "done saving sndfile\n";
            sf_close(output_sndfile);
            output_sndfile = NULL;
    ...
    case 'x':
        ...
        if (input_sndfile_count == 0) { // no command line input files, try the last recorded file
            input_sndfile_name = output_sndfile_name+s_stream+".wav";
            if (! (input_sndfile = sf_open((output_sndfile_name+s_stream+".wav").c_str(), SFM_READ, &input_sfinfo)))
                cerr << "can't open input file: " << input_sndfile_name  << endl;
            else
            {
                cerr << "opened input file: " << input_sndfile_name << endl;
                if( g_hop_size < 200 )
                    g_hop_size = 200;
            }
        }  
        else { // use command line input files
                if (! (input_sndfile = sf_open(input_sndfile_name.c_str(), SFM_READ, &input_sfinfo)))
                    cerr << "can't open input file: " << input_sndfile_name  << endl;
                else
                {
                    cerr << "opened input file: " << input_sndfile_name << endl;
...
void displayFunc( )
{
    ...
    glPushMatrix( );
        ...
        if( input_sndfile != NULL )
        {
            memset( buffer2, 0, g_buffer_size * sizeof(SAMPLE) );
            if( !sf_readf_float( input_sndfile, buffer2, g_buffer_size ) )
                sf_seek( input_sndfile, 0, SEEK_SET );
            
            pv_analyze( g_pvc2, buffer2, g_hop_size );
        }
        ...
        {
            if( other = pv_deque( g_pvc2 ) )
            {
                pv_unwrap_phase( other );
                pv_cross_synth( win[which], other );
                pv_reclaim( g_pvc2, other );
            }
            // unwrap phase
        ...
        // save buffer to disk
        if (output_sndfile != NULL)
            sf_write_float(output_sndfile, g_another_buffer, g_buffer_size);
        ...
        draw_string( .5f, 0.5f, 0.0f, str, .45f );
        ...
        draw_string( .5f, 0.3f, 0.0f, str, .45f );
        ...
        glPushMatrix();
        glTranslatef( 1.23f, 0.125f, 0.0f );
        if( g_phase_fix )
            glColor3f( 1.0f, .7f, .3f );
        else
            glColor3f( .6f, .6f, 1.0f );
        glutSolidSphere( (g_phase_fix ? .04f + .005f * fabs(cos(_inc)) : .02f), 10, 10 );
        glPopMatrix();
        ...
        if( output_sndfile )
        {
            sprintf( str, "recording to file: %s", (output_sndfile_name+s_stream+".wav").c_str() );
            draw_string( .45f, -1.3f, 0.0f, str, .40f );
            glPushMatrix();
            glTranslatef( .37f, -1.28f, 0.0f );
            glColor3f( 1.0f, .3f, .3f );
            glutSolidSphere( .04f * fabs(sin(_inc)), 10, 10 );
            glPopMatrix();
        }

        if( input_sndfile )
        {
            sprintf( str, "cross-synthesizing with file: %s", input_sndfile_name.c_str() );
            draw_string( -1.5f, -1.3f, 0.0f, str, .40f );
            glPushMatrix();
            glTranslatef( -1.58f, -1.28f, 0.0f );
